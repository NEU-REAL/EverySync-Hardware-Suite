...
def parse_args():
    ...
    #parser.add_argument('--input', type=str, default=None, help='input bag file path')
...
def main():
    args = parse_args()
    print(args)
    inputbag_path = args.inputbag_path
    outputbag_path = args.outputbag_path
    if outputbag_path is None:
        inputbag_basename, inputbag_ext = os.path.splitext(inputbag_path)  # 分离文件名与扩展名
    ...
    input_bag = rosbag.Bag(inputbag_path, 'r')
