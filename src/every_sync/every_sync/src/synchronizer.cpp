...
Synchronizer::Synchronizer(const ros::NodeHandle &nh,
  ...
  ROS_INFO("Message compiler started. Version 1.0");
  readParameters();
  ...
  imu_time_sub_ =
      nh_.subscribe(imu_time_sub_topic_, 10u,
                    &Synchronizer::imuTimeCallback, this,
                    ros::TransportHints().tcpNoDelay());
  ...
  image_sub_ = nh_.subscribe(driver_topic_, 10u,
                             &Synchronizer::imageCallback, this,
                             ros::TransportHints().tcpNoDelay());
  ...
  if (forward_camera_info_) {
    camera_info_sub_ =
        nh_.subscribe(camera_info_sub_topic_, 10u,
                      &Synchronizer::cameraInfoCallback, this,
                      ros::TransportHints().tcpNoDelay());
  ...
  ext_clk_sub_ = nh_.subscribe(ext_clk_sub_topic_, 10u,
                             &Synchronizer::ext_clkCallback, this,
                             ros::TransportHints().tcpNoDelay());
...
void Synchronizer::Imu_associateTimeStampsAndCleanUp() {
...
void Synchronizer::imageCallback(
  ...
  if (initialized_) {
    ...
    if (ros::Time::now().toSec() - init_timestamp_.toSec() <
      ...
      if (init_number_ >= kMinSuccessfullConsecutiveMatches) {
        ...
        // ROS_INFO_STREAM("every_sync node '"
...
void Synchronizer::cameraInfoCallback(
...
void Synchronizer::imageTimeCallback(
    const every_sync::TimeNumbered &image_triggered_time_msg) {
  std::lock_guard<std::mutex> mutex_lock(mutex_);
...
void Synchronizer::imuTimeCallback(
    const every_sync::TimeNumbered &image_triggered_time_msg) {
  std::lock_guard<std::mutex> mutex_lock(mutex_);
  ROS_INFO_ONCE("%s: Received first image time stamp message.",
                image_fast_pub_topic_.c_str());
...
void Synchronizer::ext_clkCallback(
    const every_sync::TimeNumbered &ext_clk_time_numbered) {
  std::lock_guard<std::mutex> mutex_lock(mutex_);
...
bool Synchronizer::readParameters() {
  ...
  if (!nh_private_.getParam("every_sync_topic", every_sync_topic_)) {
    ROS_WARN("Define a topic range where the corrected image is published.");
  } else {
    if (every_sync_topic_.back() != '/') {
      every_sync_topic_ = every_sync_topic_ + "/";
    }
    image_time_sub_topic_ = every_sync_topic_ + "image_time";
    image_fast_pub_topic_ = every_sync_topic_ + "image_raw";

    imu_time_sub_topic_ = every_sync_topic_ + "imu_time";
    imu_fast_pub_topic_ = every_sync_topic_ + "data";

    ext_clk_sub_topic_ = ext_clk_sub_topic_;
    ext_clk_pub_topic_ = ext_clk_pub_topic_;
    

    camera_info_fast_pub_topic_ = every_sync_topic_ + "camera_info";
    initialized_pub_topic_ = every_sync_topic_ + "init";
    if (publish_slow_images_) {
      image_slow_pub_topic_ = every_sync_topic_ + "slow/image_raw";
      camera_info_slow_pub_topic_ = every_sync_topic_ + "slow/camera_info";
    }
  }

  if (nh_private_.getParam("camera_info_topic", camera_info_sub_topic_)) {
    if (!camera_info_sub_topic_.empty()) {
      ROS_INFO_STREAM("A camera info topic has been provided, Synchronizer will "
