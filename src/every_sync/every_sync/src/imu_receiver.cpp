...
class ImuReciever {
public:
  ImuReciever(ros::NodeHandle &node_handle)
      : node_handle_(node_handle) {
    ROS_INFO("IMU message reciever. Version 1.0");
    readParameters();
    imu_sub_ = node_handle_.subscribe(imu_sub_topic_, 100u,
                                      &ImuReciever::imuCallback, this);
    imu_pub_ = node_handle_.advertise<sensor_msgs::Imu>(imu_pub_topic_, 100u);
    last_msg_time_ = ros::Time(0);
  }

  ~ImuReciever() { node_handle_.shutdown(); }

  void imuCallback(const every_sync::ImuMicro &imu_micro_msg) {
  ...
  void readParameters() {
    ...
    node_handle_.param("imu_sub_topic", imu_sub_topic_,
                       std::string("/every_sync/imu_micro"));
    node_handle_.param("imu_pub_topic", imu_pub_topic_,
                       std::string("/every_sync/imu"));
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "every_sync_imu_reciever");
  ros::NodeHandle node_handle("~");
  ImuReciever reciever(node_handle);
