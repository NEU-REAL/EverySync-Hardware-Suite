...
Bmi088::Bmi088(ros::NodeHandle *nh, const String &topic,
...
void Bmi088::setup()
{
  ...
  if (status < 0) 
  {
    error("BMI088.cpp:Accel Initialization Error",-1001);
  }
  status = gyro->begin();
  if (status < 0) 
  {
    error("BMI088.cppGryo Initialization Erro",-1002);
  }
  // upload feature config
  if (!writeFeatureConfig()) 
  {
    error("BMI088.cppwriteFeatureConfig Failed",-1003);
  }
  // set default ODR
  if (!setOdr(ODR_2000HZ)) 
  {
    error("BMI088.cppsetOdr Failed",-1004);
  }
  // // set default sync pin
  // if (!mapSync(PIN_3)) {
  //   while(1)
  //   {
  //     DEBUG_PRINTLN(F("mapSync Failed"));
  //   }
  // }
  ...
  //   while(1)
  //   {
  //     DEBUG_PRINTLN(F("mapDrdy Failed"));
  //   }
  // }
  ...
  //   while(1)
  //   {
  //     DEBUG_PRINTLN(F("pinModeDrdy Failed"));
  //   }
  // } 
  Imu::setupPublisher();
}
...
int16_t* Bmi088::readSensor()
{
  accel->readSensor();
  gyro->readSensor();
  static int16_t joinedData[13];
  joinedData[0] = (0xFF << 8) | 0xFF;    // Useless
  joinedData[1] = (gyro->_buffer[1] << 8) | (gyro->_buffer[0] & 0xFF);    // XGYRO_OUT
  joinedData[2] = (gyro->_buffer[3] << 8) | (gyro->_buffer[2] & 0xFF);    // YGYRO_OUT
  joinedData[3] = (gyro->_buffer[5] << 8) | (gyro->_buffer[4] & 0xFF);    // ZGYRO_OUT
  joinedData[4] = (accel->_buffer[1] << 8) | (accel->_buffer[0] & 0xFF);;    // XACCL_OUT
  joinedData[5] = (accel->_buffer[3] << 8) | (accel->_buffer[2] & 0xFF);;  // YACCL_OUT
  joinedData[6] = (accel->_buffer[5] << 8) | (accel->_buffer[4] & 0xFF);;  // ZACCL_OUT
