...
extern uint64 seq_num;
extern int ROS_FrameRate;
...
#define MAX_IMAGE_DATA_SIZE (4 * 2048 * 3072)
    ...
    enum CamerProperties
    {
        ...
        CAP_PROP_LINE_SELECTOR,      //触发线->输出信号线 
        CAP_PROP_LINE_MODE,         //输出模式 频闪输出
        CAP_PROP_LINE_SOURCE,       //输出信号源 曝光开始输出
        CAP_PROP_STROBE_ENABLE      //输出使能开关
    ...
    Camera::Camera(ros::NodeHandle &node)
    {
        ...
        node.param("LineMode", LineMode, 8);
        node.param("LineSource", LineSource, 0);
        node.param("StrobeEnable", StrobeEnable, true);
        ...
        this->set(CAP_PROP_LINE_MODE, LineMode);
        this->set(CAP_PROP_LINE_SOURCE, LineSource);
        this->set(CAP_PROP_STROBE_ENABLE, StrobeEnable);
    ...
    bool Camera::set(CamerProperties type, float value)
    {
        ...
        case CAP_PROP_LINE_MODE:
        {
            ...
            nRet = MV_CC_SetEnumValue(handle, "LineMode", value); 
        ...
        case CAP_PROP_LINE_SOURCE:
        {
            ...
            nRet = MV_CC_SetEnumValue(handle, "LineSource", value); 
        ...
        case CAP_PROP_STROBE_ENABLE:
        {

            nRet = MV_CC_SetBoolValue(handle, "StrobeEnable", value); //饱和度 默认128 最大255

            if (MV_OK == nRet)
            {
                printf("set StrobeEnable OK! value=%f\n",value);
            }
            else
            {
                printf("Set StrobeEnable Failed! nRet = [%x]\n\n", nRet);
            }
            break;
        }
    ...
    bool Camera::reset()
    {
        ...
        nRet = this->set(CAP_PROP_LINE_MODE, LineMode) || nRet;
        nRet = this->set(CAP_PROP_LINE_SOURCE, LineSource) || nRet;
        nRet = this->set(CAP_PROP_STROBE_ENABLE, StrobeEnable) || nRet;
    ...
    void *Camera::HKWorkThread(void *p_handle)
    {
        ...
        while (ros::ok())
        {
            ...
            if (nRet != MV_OK)
            {
                // if (++image_empty_count > 100)//调整空帧数量 防止因为外部触发初始化而导致的进程死亡2022.5.7 by kang
                if (++image_empty_count > 50000)
                {
                    ROS_INFO("The Number of Faild Reading Exceed The Set Value!\n");
                    exit(-1);
                    //防止因为外部触发初始化而导致的进程死亡2022.5.7 by kang
                }
                continue;
            }
